...
function normalizeHex(c, fallback="#777777"){
  ...
  let s = c.trim();
  if(!s.startsWith("#")) s = "#"+s;
  const short = /^#([0-9a-fAF]{3})$/;
  const long  = /^#([0-9a-fAF]{6})$/;
...
/* =========================
   ...
   ========================= */
const VG_LABELS = [
  "Move Forward W","Move Left A","Move Back S","Move Right D", "Jump","Crouch","Sprint","Walk Toggle", "Reload","Interact Use","Melee","Ping Marker", "Primary Fire","Secondary Fire","Aim","Holster", "Switch Weapon","Next Weapon","Prev Weapon", "Grenade Throw","Ability 1","Ability 2","Ultimate", "Map","Inventory","Scoreboard","Quest Log","Pause Menu", "Push To Talk","Emote Wheel","Screenshot","Photo Mode", "Mount Vehicle","Horn","Repair","Flashlight", "Quick Save","Quick Load","Craft","Build", "Place Marker","Zoom In","Zoom Out","Radial Menu", "Context Action","Finisher","Heal Medkit","Use Shield", "Stealth","Roll Dodge","Parry Block","Charge",
];
...
export default function App(){
  ...
  const MobileEditContent = () => {
    ...
    return (
      <div
        style={{ display: 'flex', justifyContent: 'center', height: '100%', overflowY: 'auto', padding: 16, }}
        onMouseDown={(e) => e.stopPropagation()}
        onTouchStart={(e) => e.stopPropagation()}
      >
        <div style={{ width: '100%', maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 14, }}>
          {selection.length > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, flexWrap: 'nowrap', overflowX: 'auto' }}>
              {selection.slice(0, 3).map((sid, idx) => {
                ...
                return (
                  <div key={sid} style={{ width: 80, height: 96, borderRadius: 12, background: col, boxShadow: '0 6px 16px rgba(0,0,0,.35)', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-end', padding: '6px 8px', overflow: 'hidden', }}>
                    {kd.image && kd.imageMode === 'cover' && ( <img alt="" src={kd.image} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', borderRadius: 12, zIndex: 0 }} /> )}
                    {kd.emoji && ( <div style={{ zIndex: 2, fontSize: 22, marginBottom: 4 }}>{kd.emoji}</div> )}
                    {kd.label && ( <div style={{ zIndex: 2, fontSize: fontSize, fontWeight: 700, lineHeight: 1.1, color: tp.label, maxWidth: '100%', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', }} > {kd.label} </div> )}
                  </div>
                );
              })}
              {selection.length > 3 && ( <div style={{ width: 80, height: 96, borderRadius: 12, background: 'rgba(255,255,255,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 700, color: '#fff', }} > + </div> )}
            </div>
          )}

          {selection.length > 1 && (
            <div>
              <input className="input" placeholder="Name Quick Group" value={quickGroupName} onChange={e => setQuickGroupName(e.target.value)} style={{ width: '100%', marginBottom: 8 }} />
              <button className="btn primary" onClick={() => { const name = quickGroupName.trim(); if (name) { setField('group', name); setGroupInput(name); setQuickGroupName(''); } }} style={{ width: '100%' }} > Make Group </button>
            </div>
          )}
          <div style={{ alignSelf: 'center' }}>
            <div style={{ position: 'relative', width: 120, height: 140, borderRadius: 18, background: gloss ? `radial-gradient(130% 170% at 30% 10%, rgba(255,255,255,.35), rgba(255,255,255,.08) 42%, rgba(255,255,255,.04) 60%, transparent 70%),` + ` radial-gradient(120% 170% at 70% 90%, rgba(0,0,0,.30), rgba(0,0,0,.14) 40%, transparent 70%), ` + flatBg : flatBg, boxShadow: '0 14px 28px rgba(0,0,0,.45)', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'flex-end', padding: '10px 12px', overflow: 'hidden', }}>
              {activeKey.image && activeKey.imageMode === 'cover' && ( <img alt="" src={activeKey.image} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', borderRadius: 18, zIndex: 0 }} /> )}
              {activeKey.image && activeKey.imageMode !== 'cover' && ( <img alt="" src={activeKey.image} style={{ position: 'absolute', right: 8, top: 8, width: 26, height: 26, borderRadius: 6, objectFit: 'cover', boxShadow: '0 1px 4px rgba(0,0,0,.5)', zIndex: 2 }} /> )}
              {activeKey.emoji && ( <div style={{ zIndex: 2, fontSize: 32, marginBottom: 6 }}>{activeKey.emoji}</div> )}
              {activeKey.label && ( <div style={{ zIndex: 2, fontSize: labelSize, fontWeight: 700, lineHeight: 1.1, color: pair.label }}> {activeKey.label} </div> )}
              {activeKey.sub && ( <div style={{ zIndex: 2, fontSize: subSize, color: pair.sub, opacity: 0.9 }}>{activeKey.sub}</div> )}
            </div>
          </div>
          <div className="row">
            <input className="input" placeholder="Label" value={labelInput} onChange={e => setLabelInput(e.target.value)} style={{ width: '100%' }} />
          </div>
          <div className="row">
            <input className="input" placeholder="Sub" value={subInput} onChange={e => setSubInput(e.target.value)} style={{ width: '100%' }} />
          </div>
          <div className="row">
            <input className="input" placeholder="Group" value={groupInput} onChange={e => setGroupInput(e.target.value)} style={{ width: '100%' }} />
          </div>
          <div className="row">
            <button ref={emojiButtonRef} className="btn" style={{ width: '100%', padding: '6px 10px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8 }} onClick={() => setEmojiOpen(v => !v)} >
              <SmileIcon />
              <span>{activeKey.emoji || 'Pick Emoji'}</span>
            </button>
          </div>
          {emojiOpen && createPortal( <> <ScrollbarCSS /> <div ref={pickerRef} className="emojiPanel" style={{ position: 'fixed', zIndex: 9999, ...(() => { const r = emojiButtonRef.current?.getBoundingClientRect(); const panelW = 360, pad = 8; const top = (r?.bottom ?? 0) + 6; let left = (r?.left ?? 0); const vw = window.innerWidth; if (left + panelW > vw - pad) left = Math.max(pad, vw - panelW - pad); return { top, left, width: panelW, maxHeight: 360 }; })(), overflowY: 'auto', overflowX: 'hidden', boxShadow: '0 12px 28px rgba(0,0,0,.45)', borderRadius: 12, background: 'rgba(18,23,53,0.96)', border: '1px solid rgba(255,255,255,.10)', }} onMouseDown={(e) => e.stopPropagation()} > <EmojiPicker theme="dark" autoFocusSearch={false} onEmojiClick={(data) => { setField('emoji', data.emoji); setEmojiOpen(false); emojiButtonRef.current?.focus(); }} /> </div> </>, document.body )}
          <div style={{ marginTop: 10, width: '100%' }}>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: 4 }}>Color</label>
            <input type="color" className="colorPicker" value={normalizeHex(activeKey.color || BASE.blue)} onChange={e => setField('color', e.target.value)} style={{ width: '100%', height: 36 }} />
          </div>
          <div style={{ fontWeight: 600, fontSize: 14, marginTop: 12 }}>Image</div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', width: '100%' }}>
            <label htmlFor="img-upload-mobile" className="iconBtn" title="Upload image" style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6 }} > <ImageIcon /> </label>
            <input id="img-upload-mobile" type="file" accept="image/*" style={{ display: 'none' }} onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const url = await new Promise(r => { const rd = new FileReader(); rd.onload = () => r(rd.result); rd.readAsDataURL(f); }); setField('image', url); e.target.value = ''; }} />
            <button className={`btn ${activeKey.imageMode !== 'cover' ? 'primary' : ''}`} onClick={() => setField('imageMode', 'icon')} > Icon </button>
            <button className={`btn ${activeKey.imageMode === 'cover' ? 'primary' : ''}`} onClick={() => setField('imageMode', 'cover')} > Cover </button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, width: '100%' }}>
            {keyActions.map(({ label, fn, span }, idx) => ( <button key={label} className={`btn ${span ? 'primary' : ''}`} onClick={fn} style={span ? { gridColumn: '1 / -1' } : {}} > {label} </button> ))}
          </div>
          <button className="btn" onClick={() => { setMap(prev => { const next = { ...prev }; selection.forEach(id => { const base = next[id] || {}; next[id] = { ...base, label: labelInput, sub: subInput, group: groupInput }; }); return next; }); setShowMobileEdit(false); }} style={{ marginTop: 8 }} > Continue </button>
        </div>
      </div>
    );
  };
  ...
  const MobileSettingsPanel = () => {
    return (
      <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {!isMobile && ( <label className="badge"><input type="checkbox" checked={multi} onChange={e => setMulti(e.target.checked)} /> Multi</label> )}
        ...
        <button className="btn" onClick={() => { setMap(prev => { const next = { ...prev }; Object.keys(layout).forEach(id => { if (layout[id].blank || layout[id].analog || layout[id].split) return; if (prev[id] && prev[id].group) return; next[id] = { ...(next[id] || {}), color: seededRandomColor(id + Date.now()) }; }); if (!(prev['MU'] && prev['MU'].group)) { next['MU'] = { ...(next['MU'] || {}), color: seededRandomColor('MU' + Date.now()) }; } if (!(prev['MD'] && prev['MD'].group)) { next['MD'] = { ...(next['MD'] || {}), color: seededRandomColor('MD' + Date.now()) }; } return next; }); }} > Randomize Colors </button>
        {!isMobile && ( <div style={{ display: 'flex', gap: 8 }}> <button className="iconToggle" title="Zoom Out" onClick={() => setZoom(z => Math.max(0.4, +(z - 0.1).toFixed(2)))}><ZoomOutIcon /></button> <button className="iconToggle" title="Zoom In"  onClick={() => setZoom(z => Math.min(2,   +(z + 0.1).toFixed(2)))}><ZoomInIcon /></button> </div> )}
        <button className="btn iconLabel" onClick={() => { const next = LAYOUTS[profile] || FALLBACK.positions; setLayout({ ...next }); setMap(defaultKeymapForLayout(next)); setSelection([]); setShowPopover(false); setMenu(null); }} > <ResetIcon size={16} />&nbsp;Reset Layout </button>
  ...
  const saveImage = async () => {
    ...
    if (isMobileDevice()) {
      ...
      if (newTab) {
        ...
        alert("Please allow popups for this site to view the image.");
      }
    } else {
          ...
          <div className="mobileMultiControls">
            ...
            {multi && (
              <button
                ...
                onClick={(e) => { e.stopPropagation(); if (selection.length > 0) { setShowPopover(false); setMenu(null); setShowMobileEdit(true); } }}
                onMouseDown={(e) => e.stopPropagation()}
                onTouchStart={(e) => e.stopPropagation()}
              >
                <EditIcon />
            ...
            <div style={{ position: 'relative' }}>
              <button
                className={`iconToggle`}
                title="Save images"
                onClick={(e) => { e.stopPropagation(); setShowMobileSaveMenu((v) => !v); }}
                onMouseDown={(e) => e.stopPropagation()}
                onTouchStart={(e) => e.stopPropagation()}
              >
                <SaveIcon />
              ...
              {showMobileSaveMenu && (
                <div
                  style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, background: 'rgba(18,23,53,0.96)', border: '1px solid rgba(255,255,255,.10)', borderRadius: 12, boxShadow: '0 12px 28px rgba(0,0,0,.45)', padding: 8, zIndex: 30, minWidth: 200, }}
      ...
      <div className="topbar">
        <div className="toolbarWrap">
          <div className="rowWrap">
            ...
            <label className="badge"><input type="checkbox" checked={showNumbers} onChange={e=>setShowNumbers(e.target.checked)} /> #</label>
            <button className="btn" onClick={invertLayout}>Invert</button>
            <button className="btn" title="Randomize colors" onClick={() => { setMap(prev => { const next = { ...prev }; Object.keys(layout).forEach(id => { if (layout[id].blank || layout[id].analog || layout[id].split) return; if (prev[id] && prev[id].group) return; next[id] = { ...(next[id] || {}), color: seededRandomColor(id + Date.now()) }; }); if (!(prev["MU"] && prev["MU"].group)) { next["MU"] = { ...(next["MU"] || {}), color: seededRandomColor("MU" + Date.now()) }; } if (!(prev["MD"] && prev["MD"].group)) { next["MD"] = { ...(next["MD"] || {}), color: seededRandomColor("MD" + Date.now()) }; } return next; }); }} > 🎲 </button>
          </div>
          <div className="rowWrap centerWrap">
            <input className="input titleInput titleInputXL" placeholder="Title of your game" value={gameTitle} onChange={e=>setGameTitle(e.target.value)} style={{ maxWidth:620, minWidth:260 }} />
            <div className="compactMenuWrap">
              <button className="iconToggle" title="Theme & file actions" onClick={(e)=>{ const menu = e.currentTarget.nextSibling; menu.classList.toggle("open"); }} > <ThemeIcon /> </button>
              <div className="compactMenu" onMouseLeave={(e)=>e.currentTarget.classList.remove("open")} >
                <div className="row">
                  <select className="select" defaultValue="__none__" aria-label="Apply preset theme" onChange={e=>{ const id = e.target.value; if (id === "__none__") return; setMap(prev => applyTheme(prev, layout, id)); }} >
                    <option value="__none__">Apply preset theme…</option>
                    {(() => {
                      const byCat = THEMES.reduce((acc, t) => { const k = t.cat || "Other"; (acc[k] ||= []).push(t); return acc; }, {});
                      ...
                      return orderedCats.map(cat => ( <optgroup key={cat} label={cat}> {byCat[cat].map(t => ( <option key={t.id} value={t.id}>{t.name}</option> ))} </optgroup> ));
                    })()}
                  </select>
                </div>
                <div className="row">
                  <button className="btn" onClick={()=>{ const payload = { version: 14, profile, layout, map, gameTitle, gloss, showNumbers }; const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)], { type:"application/json" })); a.download = `azeron_${profile}_theme.json`; a.click(); URL.revokeObjectURL(a.href); }} > <SaveIcon />&nbsp; Save Theme JSON </button>
                </div>
                <div className="row">
                  <button className="btn" onClick={()=>{ const inp = document.createElement("input"); inp.type="file"; inp.accept="application/json,.json"; inp.onchange = async (e)=>{ const f=e.target.files?.[0]; if(!f) return; try{ const data = JSON.parse(await f.text()); const next = data.layout ? (data.layout.positions || data.layout) : (LAYOUTS[data.profile||profile] || LAYOUTS[profile]); if(data.profile) setProfile(data.profile); if(next) setLayout({ ...next }); if(data.map) setMap(data.map); if(data.gameTitle!=null) setGameTitle(data.gameTitle); if(data.gloss!=null) setGloss(!!data.gloss); if(data.showNumbers!=null) setShowNumbers(!!data.showNumbers); }catch{ alert("Failed to load theme"); } }; inp.click(); }} > <FolderIcon />&nbsp; Load Theme JSON </button>
                </div>
                <div className="row">
            ...
            <div style={{ position: "relative" }}>
              <button className="btn primary" title="Save" onClick={(e) => { const m = e.currentTarget.nextSibling; m.style.display = m.style.display === "block" ? "none" : "block"; }} style={{ display:"inline-flex", alignItems:"center", gap:8, background:"#0b1f6f", borderColor:"rgba(255,255,255,.14)" }} > Save <span style={{ marginLeft: 8 }}>▾</span> </button>
              <div style={{ position:"absolute", top:"calc(100% + 6px)", right:0, display:"none", background:"rgba(18,23,53,0.96)", border:"1px solid rgba(255,255,255,.10)", borderRadius:12, boxShadow:"0 12px 28px rgba(0,0,0,.45)", padding:8, zIndex:10, minWidth:220 }} onMouseLeave={(e)=>{ e.currentTarget.style.display="none"; }} >
            ...
            <button className="btn iconLabel" onClick={()=>{ const next = LAYOUTS[profile] || FALLBACK.positions; setLayout({ ...next }); setMap(defaultKeymapForLayout(next)); setSelection([]); setShowPopover(false); setMenu(null); }} > <ResetIcon size={16} /> <span>Reset Layout</span> </button>
          </div>
        </div>
      </div>
      )}
          ...
          {groupLegend.length > 0 && (
            <div className="legendBar">
              ...
              <div className="legendRow">
                {groupLegend.map(([g, cols]) => { const color = cols.length ? cols[0] : '#888'; return ( <div key={g} className="legendItem"> <div className="legendDot" style={{ background: color }} /> <div className="legendText">{g}</div> </div> ); })}
              </div>
            </div>
          )}
          {showGrid && (
            <div aria-hidden style={{ position:"absolute", inset:0, pointerEvents:"none", backgroundImage: ` linear-gradient(to right, rgba(255,255,255,.06) 1px, transparent 1px), linear-gradient(to bottom, rgba(255,255,255,.06) 1px, transparent 1px) `, backgroundSize: `${VIS_GRID*displayZoom}px ${VIS_GRID*displayZoom}px, ${VIS_GRID*displayZoom}px ${VIS_GRID*displayZoom}px`, zIndex:0 }} />
          )}
          <div className="stageBox" ref={stageBoxRef} style={{ width: stageW, height: stageH, overflow: isMobile ? "auto" : "hidden", WebkitOverflowScrolling: isMobile ? "touch" : "auto" }}>
            <div id="stage-root" ref={setStageEl} className="stage" style={{ width: CANVAS_W, height: CANVAS_H, transform: `scale(${displayZoom})`, transformOrigin: "top left", position: "relative" }} onPointerDown={(e) => { if (e.button !== 0 && e.pointerType !== "touch") return; if (e.target.id === "stage-root") { if (multi) { beginLasso(e); } else { setSelection([]); setShowPopover(false); setMenu(null); } } }} >
              {lasso && ( <div style={{ position:"absolute", left: lasso.x, top: lasso.y, width: lasso.w, height: lasso.h, border:"1px dashed rgba(124,92,255,.9)", background:"rgba(124,92,255,.15)", borderRadius:6, pointerEvents:"none", zIndex:5 }} /> )}
              {Object.entries(layout).filter(([_,p])=>p.blank).map(([id,pos])=>( <div key={id} className="blank" style={{ left:pos.x, top:pos.y }} /> ))}
              {layout["MW"] && ( <SplitWheel x={layout["MW"].x} y={layout["MW"].y} up={map["MU"]} down={map["MD"]} gloss={gloss} onPick={onPick} showNumbers={showNumbers} /> )}
              {Object.entries(layout).map(([id, pos]) => { if (pos.blank || pos.analog || pos.split) return null; return ( <KeyTile key={id} id={id} data={{ ...map[id], color: colorFor(id) }} x={pos.x} y={pos.y} gloss={gloss} showNumbers={showNumbers} selected={selection.includes(id)} selection={selection.length ? selection : [id]} draggable={!locked} zoom={displayZoom} onPick={onPick} onDrag={(ids, nx, ny) => dragTo(ids, nx, ny)} onContext={(e) => openMenu(e, id)} /> ); })}
              {layout["ANALOG"] && ( <AnalogStick x={layout["ANALOG"].x} y={layout["ANALOG"].y} /> )}
              {lastSelected && showPopover && pop && (
                <div className={`popover ${pop.side}`} style={{ left:pop.x, top:pop.y, width:pop.w, height:pop.h, position:"absolute", borderRadius:14, background:"rgba(15,20,42,.90)", backdropFilter:"blur(6px)", border:"1px solid rgba(255,255,255,.08)", boxShadow:"0 10px 28px rgba(0,0,0,.35)", display:"flex", flexDirection:"column" }} onMouseDown={(e)=>e.stopPropagation()} >
                  ...
                  <div style={{ padding:12 }}>
                    <div className="row"> <label>Label</label> <input className="input" value={active.label||""} onChange={e=>setField("label", e.target.value)} /> </div>
                    <div className="row"> <label>Sub</label> <input className="input" value={active.sub||""} onChange={e=>setField("sub", e.target.value)} /> </div>
                    <div className="row"> <label>Group</label> <input className="input" value={active.group || ''} onChange={e => setField('group', e.target.value)} /> </div>
                    <div className="row"> <label>Emoji</label> <button ref={emojiButtonRef} className="btn" style={{ padding:"6px 10px", display:"inline-flex", alignItems:"center", gap:8 }} onClick={()=>setEmojiOpen(v=>!v)} > <SmileIcon /> <span>Pick Emoji</span> </button> </div>
                    {emojiOpen && createPortal( <> <ScrollbarCSS /> <div ref={pickerRef} className="emojiPanel" style={{ position:"fixed", zIndex: 9999, ...(() => { const r = emojiButtonRef.current?.getBoundingClientRect(); const panelW = 360, pad = 8; const top = (r?.bottom ?? 0) + 6; let left = (r?.left ?? 0); const vw = window.innerWidth; if (left + panelW > vw - pad) left = Math.max(pad, vw - panelW - pad); return { top, left, width:panelW, maxHeight:360 }; })(), overflowY:"auto", overflowX:"hidden", boxShadow:"0 12px 28px rgba(0,0,0,.45)", borderRadius:12, background:"rgba(18,23,53,0.96)", border:"1px solid rgba(255,255,255,.10)" }} onMouseDown={(e)=>e.stopPropagation()} > <EmojiPicker theme="dark" autoFocusSearch={false} onEmojiClick={(data)=>{ setField("emoji", data.emoji); setEmojiOpen(false); emojiButtonRef.current?.focus(); }} /> </div> </>, document.body )}
                    <div className="row" style={{ display:"flex", alignItems:"center", gap:10 }}>
                      <label>Color</label> <input type="color" className="colorPicker" value={normalizeHex(active.color||BASE.blue)} onChange={e=>setField("color", e.target.value)} style={{ width: 72, height: 42, padding: 2, borderRadius: 8, border: '2px solid #e7e9f6', background: '#fff' }} />
                      <label htmlFor="img-upload" className="iconBtn" title="Upload image" style={{ cursor:"pointer", display:"inline-flex", alignItems:"center", gap:6 }} > <ImageIcon /> </label>
                      <input id="img-upload" type="file" accept="image/*" style={{ display:"none" }} onChange={async (e)=>{ const f=e.target.files?.[0]; if(!f) return; const url = await new Promise(r=>{ const rd=new FileReader(); rd.onload=()=>r(rd.result); rd.readAsDataURL(f); }); setField("image", url); e.target.value = ""; }} />
                    </div>
                    <div className="row">
                      <label>Image Mode</label>
                      <div style={{ display:"flex", gap:8, width:"100%" }}>
                        <button className={`btn ${active.imageMode!=="cover"?"primary":""}`} onClick={()=>setField("imageMode","icon")} style={{ flex: 1 }}>Icon</button>
                        <button className={`btn ${active.imageMode==="cover"?"primary":""}`} onClick={()=>setField("imageMode","cover")} style={{ flex: 1 }}>Cover</button>
                      </div>
                    </div>
                    {active?.image && ( <div className="row"> <label>Clear Image</label> <button className="btn" onClick={()=>{ setField("image", null); setField("imageMode","icon"); }}>Remove</button> </div> )}
                  </div>
                </div>
              )}
              {menu && ( isMobile ? ( <BottomSheet open={!!menu} onClose={() => setMenu(null)}> <div style={{ padding:16, display:"flex", flexDirection:"column", gap:10 }}> <div style={{ fontWeight:700, fontSize:14, marginBottom:4 }}>Key actions</div> <button className="btn" onClick={() => doCopy()}>Copy</button> <button className="btn" onClick={() => doPaste({label:true})}>Paste Label</button> <button className="btn" onClick={() => doPaste({sub:true})}>Paste Sub</button> <button className="btn" onClick={() => doPaste({emoji:true})}>Paste Emoji</button> <button className="btn" onClick={() => doPaste({color:true})}>Paste Color</button> <button className="btn" onClick={() => doPaste({image:true})}>Paste Image</button> <button className="btn primary" onClick={() => doPaste({label:true,sub:true,emoji:true,color:true,image:true,group:true})}>Paste All</button> <button className="btn" onClick={() => setMenu(null)}>Close</button> </div> </BottomSheet> ) : createPortal( <div className="ctxMenu" style={{ position:"fixed", left:menu.x, top:menu.y, width:menu.w, height:menu.h, background:"rgba(18,23,53,0.96)", border:"1px solid rgba(255,255,255,.10)", borderRadius:12, boxShadow:"0 12px 28px rgba(0,0,0,.45)", padding:10, zIndex:9999, display:"flex", flexDirection:"column", justifyContent:"space-between" }} onContextMenu={(e)=>e.preventDefault()} onMouseDown={(e)=>e.stopPropagation()} > <div style={{ fontWeight:700, fontSize:13, marginBottom:6, opacity:.9 }}>Key actions</div> <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:6 }}> { [ ["Copy", ()=>doCopy()], ["Paste Label", ()=>doPaste({label:true})], ["Paste Sub", ()=>doPaste({sub:true})], ["Paste Emoji", ()=>doPaste({emoji:true})], ["Paste Color", ()=>doPaste({color:true})], ["Paste Image", ()=>doPaste({image:true})], ].map(([label,fn])=>( <button key={label} className="btn" onClick={fn} style={{ fontSize:12, padding:"4px 6px", lineHeight:1.1, whiteSpace:"nowrap" }} > {label} </button> )) } </div> <div style={{ display:"flex", justifyContent:"flex-end", gap:6, marginTop:6 }}> <button className="btn" onClick={()=>setMenu(null)} style={{ padding:"4px 8px", fontSize:12 }}>Close</button> <button className="btn" onClick={()=>doPaste({label:true,sub:true,emoji:true,color:true,image:true,group:true})} style={{ padding:"4px 8px", fontSize:12 }}>Paste All</button> </div> </div>, document.body ) )}
...
/* =========================
   Key component
   ========================= */
function KeyTile({ id, data, x, y, selected, selection, gloss, showNumbers, draggable, zoom, onPick, onDrag, onContext }) {
  const ref = useRef(null);
  ...
  return (
    <div
      ...
      style={mergedStyle}
      onPointerDown={(e) => { if (e.button === 0 || e.pointerType === 'touch') { onPick(id, e); } }}
      onContextMenu={(e) => onContext?.(e, id)}
      title={`Key ${id}`}
    >
      {data?.image && data?.imageMode === "cover" && ( <img alt="" src={data.image} style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", borderRadius:12, zIndex:0 }} /> )}
      {showNumbers && ( <div style={{ position:"absolute", top:6, left:8, background:"rgba(0,0,0,.85)", color:"#fff", borderRadius:"10px", padding:"2px 7px", fontSize:14, fontWeight:800, boxShadow:"0 1px 4px rgba(0,0,0,.45)", zIndex:2 }}>#{id}</div> )}
      {data?.image && data?.imageMode !== "cover" && ( <img alt="" src={data.image} style={{ position:"absolute", right:8, top:8, width:26, height:26, borderRadius:6, objectFit:"cover", boxShadow:"0 1px 4px rgba(0,0,0,.5)", zIndex:2 }} /> )}
      {data?.emoji ? <div className="emoji" style={{ zIndex:2, color: pair.label }}>{data.emoji}</div> : null}
      <div className="label" style={{ zIndex:2, color: pair.label, fontSize: labelSize, lineHeight: 1.1, overflowWrap: "anywhere", wordBreak: "break-word", hyphens: "auto" }} >
        {data?.label || "Action"}
      </div>
      <div className="sub" style={{ zIndex:2, color: pair.sub, fontSize: subSize, overflowWrap: "anywhere", wordBreak: "break-word", hyphens: "auto" }} >
        {data?.sub || ""}
      </div>
    </div>
  );
